...
export async function getAllProcesses() {
  try {
    ...
    // Fetch all workflow steps for each process
    const stepsResult = await pool.request().query(`
      ...
      INNER JOIN tblDepartments d ON pd.DepartmentID = d.DepartmentID
      ORDER BY pd.ProcessID, pd.StepOrder
    `);
    
    const steps = stepsResult.recordset;
    
    // Group steps by ProcessID
    const stepsByProcess = {};
    steps.forEach(step => {
      if (!stepsByProcess[step.ProcessID]) {
        stepsByProcess[step.ProcessID] = [];
      }
      stepsByProcess[step.ProcessID].push(step);
    });
    
    // Attach steps to each process
    const processesWithSteps = processes.map(process => ({
      ...process,
      steps: stepsByProcess[process.NumberOfProccessID] || []
    }));
    
    return processesWithSteps;
  } catch (err) {
